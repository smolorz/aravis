...
static void  // wird auch bei _flush_frames aufgerufen!
...
arv_rt_gv_stream_recv_frame(ArvStream *stream, int64_t recv_frame_timeout)
{
		...
		if (count > 0) {
			...
			for (packet_buffers = (char*)thread_data->packet_buffer; count > 0;
				...
				frame = _process_packet (thread_data, (ArvGvspPacket*)packet_buffers,
							...
							time_us);
				if (_check_frame_completion(thread_data, time_us, frame) == TRUE)
	...
	} while (frame_completed == FALSE && rt_timer_read() - start_time < recv_frame_timeout);
